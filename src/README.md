...
The entire flow is designed to be a "Straight-Through Process" (STP), meaning a user can go from application to disbursement without any manual intervention. All external checks (like PAN, Aadhaar, CIBIL, etc.) are simulated to ensure a smooth and predictable demonstration.
...
*   **Behind the Scenes:** No real SMS is sent. The app simulates sending an OTP and prepares for the next step.

### Step 3: OTP Verification

*   **Screen:** Enter OTP
*   **What the user does:** The user enters the OTP, which is always **`123456`**, and clicks **"Verify OTP"**.
*   **Behind the Scenes:**
...
*   **Behind the Scenes:** The user's consent choices are saved to a `consent_logs` subcollection within their borrower document in Firestore.
...
*   **What the user does:**
    ...
    2.  **Aadhaar e-KYC:** The user enters their Aadhaar number and clicks **"Send OTP"**. They then enter the OTP **`123456`** and click **"Verify OTP"**.
*   **Behind the Scenes:** The system simulates successful verification against government databases (NSDL/UIDAI) and updates the application status.

#### 5c. DigiLocker KYC

*   **Screen:** Step 3: DigiLocker KYC
*   **What the user does:** The user clicks **"Connect to DigiLocker"**. A popup appears where they select documents to share (e.g., Aadhaar, PAN) and click **"Share Selected Documents"**.
*   **Behind the Scenes:** The system simulates fetching documents from DigiLocker and marks the KYC as complete.

#### 5d. Credit Check & CIBIL Score

*   **Screen:** Step 4: Credit Check
*   **What the user does:**
    1.  The user clicks **"Pull My Credit Report"**.
    2.  The system simulates a CIBIL check and **immediately displays a CIBIL Score Card** showing their credit score (e.g., 780), score band (e.g., "Excellent"), and key metrics.
    3.  After reviewing their score, the user clicks **"Continue to Eligibility Result"**.
*   **Behind the Scenes:** A CIBIL report is generated. Based on the score, an automated underwriting rule **always approves the loan** to ensure the prototype flow continues.
...
*   **Behind the Scenes:** The app calculates the full amortization schedule and saves it to the loan application document in Firestore.
...
*   **What the user does:** The user reviews a loan agreement and clicks **"Sign via Aadhaar OTP"**. They enter the OTP **`123456`** to digitally sign the document.
